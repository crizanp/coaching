...
<section class="section-padding" style="background: white;">
    <div class="container">
        ...
        <div class="row">
            ...
            @php $isFeatured = $shouldHighlight && $loop->index < 3; @endphp
                ...
                <article class="practice-card-textured h-100 blog-article-card">
                    <div class="position-relative blog-cover-wrapper">
                        @if($blog->featured_image && Storage::disk('public')->exists($blog->featured_image))
                            <img src="{{ Storage::url($blog->featured_image) }}"
                                 alt="{{ $blog->title }}" class="blog-cover">
                        @else
                        ...
                        @if($isFeatured)
                            <div class="badge bg-primary position-absolute top-0 end-0 m-3">
                                <i class="fas fa-star me-1"></i>{{ __('messages.blog.featured.badge') }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="practice-card-body">
                        ...
                        <div class="practice-card-content">
                            <h4>{{ $blog->title }}</h4>
                            <div class="blog-meta">
                                <span><i class="fas fa-calendar-alt me-2"></i>{{ $blog->formatted_published_at }}</span>
                                @if(!empty($blog->reading_time))
                                    <span><i class="fas fa-clock me-2"></i>{{ $blog->reading_time }} {{ __('messages.blog.reading_time') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>

                    <p class="service-description mb-4">{{ Str::limit(strip_tags($blog->excerpt), 160) }}</p>

                    <div class="blog-card-stats mb-4">
                        <span><i class="fas fa-eye me-1"></i>{{ number_format($blog->views_count) }}</span>
                        <span><i class="fas fa-heart me-1"></i>{{ number_format($blog->likes_count) }}</span>
                    </div>

                    <div class="service-actions">
                        <a href="{{ route('blog.show', ['locale' => app()->getLocale(), 'blog' => $blog->slug]) }}"
                           class="btn btn-primary btn-sm">
                            <i class="fas fa-book-open me-2"></i>{{ __('messages.blog.read_more') }}
                        </a>
                    </div>
                </article>
            </div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="text-center fade-in empty-state">
                    ...
                    <h3 class="mb-3">{{ __('messages.blog.no_posts.title') }}</h3>
                    <p class="lead mb-4">{{ __('messages.blog.no_posts.message') }}</p>
                    @if(request('search'))
                        <a href="{{ route('blog.index', app()->getLocale()) }}" class="btn btn-primary btn-lg">
                            <i class="fas fa-undo me-2"></i>{{ __('messages.blog.no_posts.view_all') }}
                        </a>
                    @endif
                </div>
            </div>
