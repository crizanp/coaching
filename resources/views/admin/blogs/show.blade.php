...
<div class="admin-header">
    <div class="header-content">
        <h1 class="page-title">
            <i class="fas fa-eye me-3"></i>{{ $blog->title }}
        </h1>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p>{{ $blog->meta_title ?: $blog->title }}</p>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="mb-3">
                        ...
                        <p>{{ $blog->meta_description ?: $blog->excerpt }}</p>
                    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="mb-1">{{ $blog->created_at->format('M d, Y \a\t g:i A') }}</p>
                        
                        <h6 class="text-muted mt-3">Last Updated:</h6>
                        <p class="mb-1">{{ $blog->updated_at->format('M d, Y \a\t g:i A') }}</p>
                        
                        @if($blog->published_at)
                            <h6 class="text-muted mt-3">Published:</h6>
                            <p class="mb-1">{{ $blog->published_at->format('M d, Y \a\t g:i A') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="stats-grid">
                        <div class="stat-item">
                            <div class="stat-value">{{ number_format($blog->views_count) }}</div>
                            <div class="stat-label">Views</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">{{ $blog->likes_count }}</div>
                            <div class="stat-label">Likes</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">{{ $blog->dislikes_count }}</div>
                            <div class="stat-label">Dislikes</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">{{ $blog->reading_time }}</div>
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        ...
                        <form action="{{ route('admin.blogs.toggle-publish', $blog) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-{{ $blog->is_published ? 'warning' : 'success' }} w-100">
                                <i class="fas fa-{{ $blog->is_published ? 'eye-slash' : 'globe' }} me-2"></i>
