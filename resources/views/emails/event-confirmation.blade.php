...
<head>
    ...
    <title>{{ $type === 'confirmed' ? 'Confirmation' : ($type === 'cancelled' ? 'Annulation' : 'Mise à jour') }} de votre inscription</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p>{{ $event->getTranslation('title', 'fr') }}</p>
        </div>

        <!-- Content -->
        <div class="content">
            <p style="font-size: 16px; margin-bottom: 20px;">Bonjour {{ $application->name }},</p>
            ...
            <div class="event-details">
                <div class="event-title">{{ $event->getTranslation('title', 'fr') }}</div>
                
                @if($event->getTranslation('description', 'fr'))
                    <p style="margin-bottom: 20px; color: #666;">{{ $event->getTranslation('description', 'fr') }}</p>
                @endif

                <div class="detail-row">
                    <span class="detail-label">Type :</span>
                    <span class="detail-value">{{ ucfirst($event->type ?: 'Atelier') }}</span>
                </div>

                @if($event->event_date)
                    <div class="detail-row">
                        <span class="detail-label">Date :</span>
                        <span class="detail-value">{{ $event->event_date->format('d/m/Y à H:i') }}</span>
                    </div>
                @endif
                ...
                @if($event->getTranslation('location', 'fr'))
                    <div class="detail-row">
                        <span class="detail-label">Lieu :</span>
                        <span class="detail-value">{{ $event->getTranslation('location', 'fr') }}</span>
                    </div>
                @endif
                ...
                @if($event->price)
                    <div class="detail-row">
                        ...
                        <span class="detail-value">€{{ number_format((float)$event->price, 2) }}</span>
                    </div>
                @else
                    <div class="detail-row">
                        <span class="detail-label">Prix :</span>
            ...
            <!-- Workshop Content -->
            @if($event->getTranslation('content', 'fr') && $type === 'confirmed')
                <div class="message">
                    <h3 style="margin-bottom: 15px; color: #1976d2;">À propos de cet atelier :</h3>
                    <div style="white-space: pre-line;">{{ $event->getTranslation('content', 'fr') }}</div>
                </div>
            @endif

            <!-- Additional Notes -->
            @if($application->notes && $type === 'confirmed')
                <div class="message">
                ...
                <div style="margin-top: 30px;">
                    ...
                    <ul style="padding-left: 20px; color: #555;">
                        ...
                        @if($event->registration_deadline)
                            <li>Si vous devez annuler, merci de nous prévenir avant le {{ $event->registration_deadline->format('d/m/Y') }}</li>
                        @endif
